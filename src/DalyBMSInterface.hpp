...
struct Interface {
    ...
    explicit Interface(const Config& conf)
        : config(conf), hardware(config.serialId), connector(hardware), manager(config.manager, connector) {
...
private:
    template<auto MethodPtr> void forEachManager() {
        for (auto& manager : managers)
            (manager->*MethodPtr)();
    }
    template<auto MethodPtr> void forEachManager() const {
        for (auto& manager : managers)
            (manager->*MethodPtr)();
    }
};
